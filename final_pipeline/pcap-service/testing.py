...
class Packet:
    ...
    list_of_parameters = (
        general_headers + ip_headers + protocols + source_ip_type + destination_ip_type
    )
    ...
    def extract_data(self):
        ...
        for layer in self.packet.layers():
            ...
            if layer_name == "IP":
                ...
                self.check_if_ip_is_internal_or_external(
                    self.packet_dict["ip.src"], kind="source"
                )

                self.packet_dict["ip.dst"] = packet_layer.fields.get("dst")
                self.check_if_ip_is_internal_or_external(
                    self.packet_dict["ip.dst"], kind="destination"
                )
